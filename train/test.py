...
def inference(sess, ops, pc, one_hot_vec, batch_size):
    ...
    for i in range(int(num_batches)):
        ...
        heading_logits[i*batch_size:(i+1) *
                       batch_size, ...] = batch_heading_scores
        heading_residuals[i*batch_size:(i+1) *
                          batch_size, ...] = batch_heading_residuals
        size_logits[i*batch_size:(i+1)*batch_size, ...] = batch_size_scores
        size_residuals[i*batch_size:(i+1) *
                       batch_size, ...] = batch_size_residuals
        ...
        batch_scores = np.log(mask_mean_prob) + \
            np.log(heading_prob) + np.log(size_prob)
